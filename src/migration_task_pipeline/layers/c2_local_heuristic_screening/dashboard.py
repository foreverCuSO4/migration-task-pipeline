"""Terminal dashboard for Stage C2 local heuristic screening."""
...
def dashboard_lines(snapshot: dict[str, object], *, width: int) -> list[str]:
    ...
    run_completed = as_int(snapshot.get("promoted_count")) + as_int(snapshot.get("maybe_count"))
    run_completed += as_int(snapshot.get("rejected_count")) + as_int(snapshot.get("failed_count"))
    ...
    repo_key = str(snapshot.get("repo_key") or "")
    decision = str(snapshot.get("decision") or "")
    c2_score = snapshot.get("c2_score")

    lines = [
        "Layer C2 local heuristic screening",
        f"Progress   {progress}",
        f"Elapsed    {format_duration(elapsed)}   Rate {rate_per_min:.2f} repos/min   ETA {format_duration(eta)}",
        (
            "Decisions  "
            f"promote {as_int(snapshot.get('promoted_count'))}   "
            f"maybe {as_int(snapshot.get('maybe_count'))}   "
            f"reject {as_int(snapshot.get('rejected_count'))}   "
            f"failed {as_int(snapshot.get('failed_count'))}"
        ),
        (
            "Input      "
            ...
            f"rejected {input_counts.get('rejected', 0)}   "
            f"failed {input_counts.get('failed', 0)}"
        ),
        f"Output     pending {output_counts.get('pending', 0)}   done {output_counts.get('done', 0)}",
        fit_line(f"Current    {worker_id} {repo_key}".rstrip(), width),
        fit_line(f"Phase      {phase}", width),
    ]
    if decision:
        lines.append(f"Last       {decision}   score {format_score(c2_score)}")
...
def phase_text(snapshot: dict[str, object]) -> str:
    ...
    if event == "item_claimed":
        return "claimed repo"
    if event == "repo_scanned":
        return "repo scanned"
    if event == "item_done":
        return "repo promoted to D"
    if event == "item_rejected":
        return "repo rejected"
